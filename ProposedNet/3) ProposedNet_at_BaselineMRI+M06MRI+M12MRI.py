...
normal_scan_paths = glob.glob('/home/ubuntu/nasir/pytorch_3d_resnet_data/three_steps/1.CN/*.*')
abnormal_scan_paths = glob.glob('/home/ubuntu/nasir/pytorch_3d_resnet_data/three_step/2.AD/*.*')
all_pths = normal_scan_paths + abnormal_scan_paths

standardize_features = '/home/ubuntu/nasir/pytorch_3d_resnet_data/three_steps/standardized_features_with.csv'
...
class Dataset_Early_Fusion(Dataset):
    def __init__(self, df, indices):
        ...
        self.features_df = self.all_features_df.drop(columns=["Unnamed: 0", "Unnamed: 0.1", "Subject_id"])
...
class ProposedNet(nn.Module):
    ...
    def forward(self, x1, x2, x3, f):
        ...
        sequences = concat.reshape(-1, 3, 13824)
...
def train(train_dataloader, val_dataloader, fold):
    ...
    for e in t:    
        ...
        if(old_acc==val_acc) and (old_auc<val_auc):
            ...
            save_path = '/home/ubuntu/nasir/pytorch_3d_resnet_data/scripts/Repeated_Experiments/Proposed_network/CNN_RNN_three_steps_without_features_fold_' + str(fold)
...
for indx, (train_index, test_index) in enumerate(skf.split(all_pths, all_labels)):  
    ...
    train_path = '/home/ubuntu/nasir/pytorch_3d_resnet_data/three_steps/Train_Fold' + str(indx) + '.csv'
    ...
    valid_path = '/home/ubuntu/nasir/pytorch_3d_resnet_data/three_steps/Valid_Fold' + str(indx) + '.csv'
...
save_path = '/home/ubuntu/nasir/pytorch_3d_resnet_data/scripts/Repeated_Experiments/Proposed_network/CNN_RNN-Three_Steps_without_features.npy'
np.save(save_path, z)
...
three_step_mri = np.load('/home/ubuntu/nasir/pytorch_3d_resnet_data/scripts/Repeated_Experiments/Proposed_network/CNN_RNN-Three_Steps_without_features.npy',allow_pickle=True)

mean_std(three_step_mri.item().get('net prediction: '), three_step_mri.item().get('ture labels: '))   
