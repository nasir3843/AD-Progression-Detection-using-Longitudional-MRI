...
normal_scan_paths = glob.glob('/home/ubuntu/nasir/pytorch_3d_resnet_data/single_step/1.CN/*.*')
abnormal_scan_paths = glob.glob('/home/ubuntu/nasir/pytorch_3d_resnet_data/single_step/2.AD_/*.*')
all_pths = normal_scan_paths + abnormal_scan_paths

standardized_features = '/home/ubuntu/nasir/pytorch_3d_resnet_data/single_step/standardized_features.csv'

df = pd.read_csv(standardized_features)
features = []
...
class Dataset_Early_Fusion(Dataset):
    def __init__(self, df, indices):
        ...
        self.all_features_df = pd.read_csv(standardized_features)
...
class ProposedNet(nn.Module):
    ...
    def forward(self, x, f):
        ...
        x = x.view(x.size(0), -1)        
        sequences = x.reshape(-1, 2, 6912)        
        out = self.rnn(sequences)
        out = self.dense_layer1(out)            
        dens_output = self.dense_layer2(f)     
...
def train(train_dataloader, val_dataloader, fold):
    ...
    for e in t:    
        ...
        net.train()
        for step, (img1, features, label) in enumerate(train_dataloader):            
        ...
        ap = metrics.average_precision_score(y_true, torch.max(y_pred, 1)[1])

        net.eval()
        full_path = []
        with torch.no_grad():
            for step, (img1, features, label) in enumerate(val_dataloader):
        ...
        if(old_acc==val_acc) and (old_auc<val_auc):
            ...
            save_path = '/home/ubuntu/nasir/pytorch_3d_resnet_data/scripts/Repeated_Experiments/Exp_with_demographics/CNN_RNN_single_step_cognitive_features_fold_' + str(fold)
...
for indx, (train_index, test_index) in enumerate(skf.split(all_pths, all_labels)):  
    ...
    train_path = '/home/ubuntu/nasir/pytorch_3d_resnet_data/three_steps/Train_Fold_cog_feat119' + str(indx) + '.csv'
    ...
    valid_path = '/home/ubuntu/nasir/pytorch_3d_resnet_data/three_steps/Train_Fold_cog_feat119' + str(indx) + '.csv'
...
np.save('/home/ubuntu/nasir/pytorch_3d_resnet_data/scripts/Repeated_Experiments/Exp_with_demographics/CNN_RNN-single_step_cognitive_features.npy', 
       z)

def mean_std(test_y_pred, test_y_true):    
...
v = np.load('/home/ubuntu/nasir/pytorch_3d_resnet_data/scripts/Repeated_Experiments/Exp_with_demographics/CNN_RNN-single_step_cognitive_features.npy', allow_pickle=True)
pred = v.item().get('net prediction: ')
lab = v.item().get('true labels: ')
mean_std(pred, lab.long())
