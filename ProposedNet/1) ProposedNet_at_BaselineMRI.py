...
# dumy_img1 =torch.randn(2,1,110, 110, 110)
# num_features = torch.randn(2, 17)
# z = net(dumy_img1.to(device), num_features.to(device))
# print(z)
...
def train(train_dataloader, val_dataloader, fold):
    ...
    for e in t:    
        ...
        if(old_acc==val_acc) and (old_auc<val_auc):
            ...
            save_path = '/home/ubuntu/nasir/pytorch_3d_resnet_data/scripts/Repeated_Experiments/Proposed_network/CNN_RNN_single_step_without_features_fold_' + str(fold)
...
for indx, (train_index, test_index) in enumerate(skf.split(all_pths, all_labels)):  
    ...
    train_path = './Train_Fold' + str(indx) + '.csv'
    ...
    valid_path = './Valid_Fold' + str(indx) + '.csv'
...
np.save('./proposed_net_MRI_baseline.npy', z)
...
single_step_mri = np.load('./proposed_net_MRI_baseline.npy',allow_pickle=True)
