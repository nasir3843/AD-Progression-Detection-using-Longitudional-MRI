...
normal_scan_paths = glob.glob('/home/ubuntu/nasir/pytorch_3d_resnet_data/three_step_oversampling/1.CN/*.*')
abnormal_scan_paths = glob.glob('/home/ubuntu/nasir/pytorch_3d_resnet_data/three_step_oversampling/2.AD/*.*')
all_pths = normal_scan_paths + abnormal_scan_paths

standardize_features = '/home/ubuntu/nasir/pytorch_3d_resnet_data/three_step_oversampling/standardized_features_with_oversampling2.csv'
...
class Dataset_Early_Fusion(Dataset):
    def __init__(self, df, indices):
        ...
        columns_to_drop = ['Unnamed: 0', 'Unnamed: 0.1', 'FDG', 'TAU','PTAU',
                           'APOE4','CDRSB',
...
class ResNet3D(nn.Module):
    def __init__(self, num_classes=2, input_shape=(1,110,110,110)): # input: input_shape:	[num_of_filters, kernel_size] (e.g. [256, 25])
        ...
        self.dense_layer2 = nn.Sequential(nn.Linear((512 * 3) * 2, 256),
                                 nn.ReLU(),
                                 nn.Dropout(0.4),
                                 nn.Linear(256, 128),
                                 nn.ReLU())
    ...
    def forward(self, x1, x2, x3, f):
        ...
        sequences = concat.reshape(-1, 3, 1024)
...
#                                                   cycle_momentum=False)
    ...
    for e in t:    
        ...
        for step, (img, features, label) in enumerate(train_dataloader):            
            ...
            opt.zero_grad()
            out = net(img1, img2, img3, features)            
        ...
        if(old_acc==val_acc) and (old_auc<val_auc):
            ...
            save_path = '/home/ubuntu/nasir/pytorch_3d_resnet_data/scripts/Repeated_Experiments/resnet_three_steps_with_demographic_fold_' + str(fold)
...
for indx, (train_index, test_index) in enumerate(skf.split(all_pths, all_labels)):  
    ...
    train_path = '/home/ubuntu/nasir/pytorch_3d_resnet_data/three_steps/Train_Fold_three_resnt' + str(indx) + '.csv'
    ...
    valid_path = '/home/ubuntu/nasir/pytorch_3d_resnet_data/three_steps/Valid_Fold_three_resnt' + str(indx) + '.csv'
...
np.save('/home/ubuntu/nasir/pytorch_3d_resnet_data/scripts/Repeated_Experiments/resnet_three_steps_with_demographic.npy', 
       z)
...
single_step_mri = np.load('/home/ubuntu/nasir/pytorch_3d_resnet_data/scripts/Repeated_Experiments/Proposed_network/resnet_three_steps_Cognitive_features.npy',allow_pickle=True)
